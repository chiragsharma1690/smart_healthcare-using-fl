...
from functions.audio_segmentor import audio_segmentor
from functions.acoustic_feature_extraction import feature_extractor_train2021, feature_extractor_train2020
...
class Initializer:
    def __init__(self, num_clients, num_servers, iterations):
        ...
        # audio_segmentor()         # for adress 2021

        root_dir2021 = "C:/Users/chira/Downloads/Federated Learning Project/my_code_exp/ADReSSo21"
        train_audio_dataset_path = root_dir2021 + "/diagnosis/train/audio"
        train_mmse = root_dir2021 + "/diagnosis/train/shuffled_adresso-train-mmse-scores.csv"

        df = pd.read_csv(train_mmse)

        # X, y = feature_extractor_train2021(df)
        X, y = feature_extractor_train2020()

        print("X:", X)
        print("y: ", y)

        # X, y = df_new['adressfname'], df_new['dx']
